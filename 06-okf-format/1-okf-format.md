...
*Du hast das OKF-[[Frontmatter]] (`type`, `status`, `sources` ...) in Kapitel 4 schon in Aktion gesehen — deine KI hat damit gearbeitet, ohne dass du wissen musstest, was genau dahintersteckt. Dieses Kapitel ist die Vertiefung für alle, die es jetzt genauer wissen wollen. Kein Muss, um mit dem Workshop fertig zu sein — aber ein spannender Blick unter die Haube.*
...
Praktisch heißt das: Man schreibt frei im Journal, und nur wenn eine Idee es wert ist, bekommt sie beim „Befördern" ins Second Brain einen `type`, vielleicht `status: draft`, später `verified`. Genau diesen Übergang — von unstrukturierter Notiz zu OKF-Format — behandelt Kapitel 4 im Detail.
